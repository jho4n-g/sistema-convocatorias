...
export default function ConvocatoriaPage() {
  ...
  const columns = useMemo(
    () => [
      ...
      {
        accessorKey: 'experiencia_general',
        header: 'EXPERIENCIA GENERAL',
      },

      {
        accessorKey: 'cantidad_personal',
        header: 'CANTIDAD PERSONAL',
      },
      {
        accessorKey: 'nombre_servicio',
        header: 'Servicio revisor',
      },
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex flex-col gap-2">
            {tienePermiso('convocatoria.editar') && (
              <button
                type="button"
            ...
            {tienePermiso('postulantes.ver') && (
              <button
                type="button"
                className="rounded-xl bg-slate-200 px-3 py-2 text-black hover:bg-slate-300 border border-slate-500"
                onClick={() => {
                  navigate(`/ver-postulantes/${row.original.id}`);
                }}
              >
                Ver postulantes
              </button>
  ...
  return (
    <>
      <div className="mb-6 flex items-center justify-between">
        <h2 className="text-xl font-semibold">GESTION DE CONVOCATORIAS</h2>
        {tienePermiso('convocatoria.crear') && (
          <button
            className="
            rounded-xl
            bg-emerald-800
