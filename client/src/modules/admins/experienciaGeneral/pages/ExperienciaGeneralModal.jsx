...
//
import { ExperienciaGeneralServices as Servs } from '../experienciaGeneral.services';
import {
  experienciaGeneralSchema,
  experienciaGeneralUpdateSchema,
} from '../experienciaGeneral.schema';
...
export default function CentroMedicoModal({
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (isEdit) {
          const data = {
            meses_experiencia: dataRow?.meses_experiencia ?? '',
            label: dataRow?.label ?? '',
          };
  ...
  const handleValidation = () => {
    ...
    const result = isEdit
      ? experienciaGeneralUpdateSchema.safeParse(payload)
      : experienciaGeneralSchema.safeParse(payload);
        ...
        >
          ...
          <div className="bg-white rounded-2xl shadow p-4 sm:p-6 mb-2">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-4 sm:gap-6">
              ...
              <div className="md:col-span-1 lg:col-span-12">
                <InputField
                  label="Ingreso experiencia general en meses (solo numero)"
                  name="meses_experiencia"
                  placeholder="Ejemplo: 33"
                  type="number"
                  value={form.meses_experiencia ?? ''}
                  onChange={handleChange}
                  error={error.meses_experiencia}
                />
              </div>
            </div>
          </div>
