...
import { ServiceCentroServices as Servs } from '../ServicioCentro.services';
import { CentroMedicoServices } from '../../CentroMedicos/CentroMedico.services';
...
export default function CentroMedicoModal({
  ...
  const [centrosMedico, setCentrosMedico] = useState([]);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        if (!open) return;
        setLoading(true);
        const responseCentrosMedico = await CentroMedicoServices.getAllSelect();

        if (!responseCentrosMedico.ok) {
          throw new Error('No se puede cargar los centros medicos');
        }

        setCentrosMedico(responseCentrosMedico.data);

        setError({});
        setForm(initialForm());

        if (isEdit && id) {
          const responseId = await Servs.getId(id);
          if (!responseId.ok) {
            throw Error('No se pudo cargar el serivicion centro');
          }

          setForm(responseId.data);
          setOriginalForm(responseId.data);
        }
      } catch (e) {
        toast.error(
          e instanceof Error
  ...
  const handleValidation = () => {
    ...
    const result = isEdit
      ? servicioCentroSchema.safeParse(payload)
      : servicioCentroUpdateSchema.safeParse(payload);
        ...
        >
          ...
          <div className="bg-white rounded-2xl shadow p-4 sm:p-6 mb-2">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-4 sm:gap-6">
              <div className="md:col-span-1 lg:col-span-12 space-y-4">
                <InputField
                  ...
                  error={error.nombre_servicio}
                />
                <Select
                  label="Seleccionar centro medico"
                  name="centro_medico_id"
                  options={centrosMedico}
                  value={form?.centro_medico_id ?? ''}
                  onChange={handleChange}
                  error={error.centro_medico_id}
                />
                <Select
                  label="Seleccionar estado del servicio"
                  name="estado"
                  options={[
