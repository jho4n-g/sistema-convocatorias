...
import ServicioCentroModal from './ServicioCentroModal';

import { usePermisos } from '../../../../hooks/usePermisos';

export default function ServicioCentroPage() {
  ...
  const handleCambiarEstado = async (id) => {
    try {
      setLoading(true);
      const response = await Servs.CambioEstado(id);
      if (!response.ok) {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: ({ getValue }) => (
          <span
            className={`p-3 rounded-2xl text-white ${getValue() ? 'bg-emerald-700' : 'bg-red-700'} `}
          >
...

      {
        ...
        cell: ({ row }) => (
          <div className="flex flex-col gap-2">
            {tienePermiso('servicioCentro.editar') && (
              <button
                type="button"
            ...
            {tienePermiso('servicioCentro.deshabilitar') && (
              <button
                type="button"
                className="rounded-xl bg-slate-200 px-3 py-2 text-black hover:bg-slate-300 border border-slate-500"
                onClick={() => handleCambiarEstado(row.original.id)}
              >
                {row.original.estado ? 'Deshabilitar' : 'Habilitar'}
              </button>
            )}
          </div>
        ),
    ...
    <>
      <div className="mb-6 flex items-center justify-between">
        <h2 className="text-xl font-semibold">
          Gestion de Servicios de los Centros Medicos
        </h2>
